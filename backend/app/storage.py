...
def load_export(path: Path | str) -> dict[str, Any]:
    ...
    if isinstance(raw, list) and raw and "gastosapp_export" in raw[0]:
        return raw[0]["gastosapp_export"]
    if isinstance(raw, dict) and "gastosapp_export" in raw:
        return raw["gastosapp_export"]
    if isinstance(raw, dict):
        return raw
    raise ValueError("Formato de export no reconocido")


def import_supabase_export(conn: sqlite3.Connection, export: dict[str, Any], admin_password: str | None = None) -> dict[str, Any]:
    init_db(conn)
    now = utc_now()
    ...
    with conn:
        for row in export.get("app_users", []):
            email = (row.get("email") or "").lower()
            role = row.get("role") or ("admin" if email == ADMIN_EMAIL else "user")
            conn.execute(
                """
            ...
            if email == ADMIN_EMAIL:
                set_user_password(conn, str(row["id"]), admin_password)

        for row in export.get("app_workspaces", []):
            conn.execute(
                """
                ...
                (
                    str(row["id"]),
                    row.get("name") or "Base sin nombre",
                    row.get("created_by"),
                    1 if row.get("is_personal") else 0,
                    row.get("created_at") or now,
                    row.get("updated_at") or now,
                ),
            )

        for row in export.get("app_workspace_members", []):
            conn.execute(
                """
        ...
        for table in SNAPSHOT_TABLES:
            for row in export.get(table, []):
                conn.execute(
                    f"""
                    insert into {table}(id,workspace_id,data,created_at,updated_at)
                    values (?,?,?,?,?)
                    on conflict(id) do update set
                    ...
                    """,
                    (
                        str(row.get("id") or secrets.token_urlsafe(12)),
        ...
        conn.execute("insert or replace into migration_meta(key,value) values (?,?)", ("source_exported_at", str(export.get("exported_at") or "")))
        conn.execute("insert or replace into migration_meta(key,value) values (?,?)", ("migrated_at", now))

    report = migration_report(conn, export)
    if generated_password:
        report["admin_temp_password"] = generated_password
    return report
